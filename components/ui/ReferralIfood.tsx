...
export default function ReferralIfood({ image, title, description, cta }: ReferralIfoodProps) {
  return (
    <section className="w-full bg-black md:pb-28 md:pt-28 lg:px-24">
      <div className="xl:container mx-4 lg:mx-[120px] xl:mx-auto flex justify-center rounded-[24px] relative z-1 black-green">
        <div className="flex flex-col-reverse items-center md:grid grid-cols-[1fr_200px] gap-6 px-6 pt-16 pb-20">
            ...
            <div className="flex gap-4">
              {cta?.map(({ color = 'none', href = '', label = '', icon }) => (
                <a
                  ...
                  ${color === 'green' ? 'bg-[#02F67C] text-black' : 'bg-transparent text-black'}
                  `}
                  href={href}
                >
